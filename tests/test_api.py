...
class APITestCase(unittest.TestCase):
    ...
    def test_404(self):
        response = self.client.get(
            "/wrong/url",
            headers=self.get_api_header("email", "password")
        )
    ...
    def test_bad_auth(self):
        ...
        response = self.client.get(
            url_for("api.get_posts"),
            headers=self.get_api_header("john@test.com", "dog")
        )
    ...
    def test_posts(self):
        ...
        response = self.client.post(
            ...
            headers=self.get_api_header(u.email, "cat"),
        ...
        response = self.client.get(
            url,
            headers=self.get_api_header("john@test.com", "cat")
        )
        self.assertTrue(response.status_code==200)
        json_response = json.loads(response.data.decode("utf-8"))
